...
@section('content-header')
    <h1>
        {{ trans('idownload::categories.title.edit category') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.idownload.category.index') }}">{{ trans('idownload::categories.title.categories') }}</a></li>
        <li class="active">{{ trans('idownload::categories.title.edit category') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.idownload.category.update', $category->id], 'method' => 'put']) !!}
    <div class="row">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('idownload::admin.categories.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
